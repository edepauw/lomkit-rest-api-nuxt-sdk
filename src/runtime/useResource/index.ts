...
const useResource = <T>(
  resourceName: string,
  preset: IResourcePreset<T> = {}
) => {
  // @ts-ignore
  const apiClient = useNuxtApp().$lomkitRestClient.getApiClient(
    preset.apiClientSlug
  );
  ...
  return {
     ...
     */
    findOne: async (request: ISearchQuery<T> = {}): Promise<T | null> =>
      await findOne<T>(
        { ...preset.search, ...request },
        resourceUrl,
        requestInit()
      ),
    ...
    findOneById: async (
      id: number | string,
      request: IFindOneByIdRequest<T> = {}
    ): Promise<T | null> =>
      await findOne<T>(
        { filters: [{ field: "id", value: id }], ...preset.search, ...request },
        resourceUrl,
        requestInit()
      ),
     ...
     *   ])
     */
    mutate: async (mutations: IMutateRequest<T>[]): Promise<IMutateResponse> =>
      await mutate<T>(mutations, resourceUrl, requestInit()),
    ...
    actions: async (
      ...
      await actions(actionName, params, resourceUrl, requestInit()),
     ...
     */
    remove: async (ids: number[]): Promise<any> =>
